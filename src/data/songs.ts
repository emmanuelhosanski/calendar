import { Song } from '../types';

export const songs: Song[] = [
  {
    id: 1,
    title: "La grenade",
    artist: "Clara Luciani",
    audioUrl: "/songs/day1.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Hey you
    What are you looking at?
    You've never seen a woman
    Who is fighting?
    Follow me
    In the pale city
    And I will show you
    How I bite, how I bark
    
    Take care
    Under my breast, the pomegranate
    Under my breast, there, look
    Under my breast, the pomegranate
    `
    },
    {
    id: 2,
    title: "Uptown Funk",
    artist: "Bruno Mars",
    audioUrl: "/songs/day2.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Ce coup, ce froid glacial
    Michelle Pfeiffer, cet or blanc
    Celui-ci pour les filles du quartier
    Ce sont de bonnes filles, des chefs-d'œuvre hétérosexuels
    Styliser, vouloir, vivre en ville
    J'ai des Chucks avec Saint Laurent
    Je dois m'embrasser, je suis si jolie
    
    J'ai trop chaud (putain)
    (Doh)
    Euh, appelle la police et le pompier
    (Doh-doh-doh, doh-doh-doh, doh-doh)
    J'ai trop chaud (putain)
    (Doh-doh-doh, doh-doh-doh, doh-doh)
    Donne envie à un dragon de prendre sa retraite, mec
    J'ai trop chaud (putain)
    (Doh-doh-doh, doh-doh-doh, doh-doh)
    Dis mon nom, tu sais qui je suis
    J'ai trop chaud (putain)
    (Doh-doh-doh, doh-doh, doh, doh)
    Et mon groupe parle de cet argent, décompose-le
    
    Les filles, allez vous alléluia (Woo)
    Les filles, allez vous alléluia (Woo)
    Les filles, allez vous alléluia (Woo)
    Parce que le funk des quartiers chics va te le donner (Woo)
    (Parce que le funk des quartiers chics va te le donner)
    Parce que le funk des quartiers chics va te le donner
    Samedi soir et nous sommes sur place
    Ne me crois pas, regarde juste, allez
    `
    },
    {
    id: 3,
    title: "You're the one that I want",
    artist: "John Travolta and Olivia Newton John",
    audioUrl: "/songs/day3.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    J'ai des frissons, ils se multiplient
    Et je perds le contrôle
    Parce que le pouvoir que tu fournis
    C'est électrisant
    
    Tu ferais mieux de te mettre en forme, parce que j'ai besoin d'un homme
    Et mon cœur est tourné vers toi (Et mon cœur est tourné vers toi)
    Tu ferais mieux de te mettre en forme, tu comprendras mieux
    Dans mon cœur, je dois être vrai
    Il ne me reste plus rien, il ne me reste plus rien à faire
    
    Tu es celui que je veux (Tu es celui que je veux)
    Ooh, ooh, ooh, chérie
    Celui que je veux (Tu es celui que je veux)
    Ooh, ooh, ooh, chérie
    Celui que je veux (Tu es celui que je veux)
    Ooh, ooh, ooh
    Celui dont j'ai besoin (Celui dont j'ai besoin)
    Oh, oui, en effet (Oui, en effet)
    `
    },
    {
    id: 4,
    title: "Whenever, wherever",
    artist: "Shakira",
    audioUrl: "/songs/day4.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Heureusement que tu es né si loin alors
    On pourrait tous les deux se moquer de la distance
    Heureusement que j'aime un pays étranger pour
    Le fait heureux de ton existence
    Bébé, je gravirais les Andes uniquement
    Pour compter les taches de rousseur sur ton corps
    Je n'aurais jamais pu imaginer qu'il n'y avait que
    Dix millions de façons d'aimer quelqu'un
    
    Le-do-lo-le-lo-le
    Le-do-lo-le-lo-le
    Tu ne vois pas ?
    je suis à tes pieds
    
    N'importe quand, n'importe où
    Nous sommes censés être ensemble
    Je serai là et tu seras près
    Et c'est l'affaire, ma chère
    Par ailleurs, ci-dessous
    Tu n'auras jamais à te demander
    On peut toujours jouer à l'oreille
    Mais c'est le marché, ma chère
    `
    },
    {
    id: 5,
    title: "Wonderwall",
    artist: "Oasis",
    audioUrl: "/songs/day5.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Aujourd'hui sera le jour où ils te le rendront
    À présent, tu devrais avoir réalisé d'une manière ou d'une autre ce que tu dois faire
    Je ne crois pas que quiconque ressent ce que je ressens pour toi maintenant
    
    Et toutes les routes que nous devons parcourir sont sinueuses
    Et toutes les lumières qui nous y conduisent sont aveuglantes
    Il y a beaucoup de choses que je voudrais te dire
    Mais je ne sais pas comment
    
    Parce que peut-être
    Tu seras celui qui me sauvera
    Et après tout
    Tu es mon mur des merveilles
    `
    },
    {
    id: 6,
    title: "Ma philosopohie",
    artist: "Amel Bent",
    audioUrl: "/songs/day6.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    I only have one philosophy
    To be accepted as I am
    Despite everything I'm told
    I remain with my fist raised
    For better or worse
    I am mixed race but not a martyr
    I move forward with a light heart, but always with my fist raised
    
    Raise your head, puff out your chest
    Constantly redouble our efforts
    Life leaves me no choice
    I am the ace who beats the king
    Despite our sorrows, our differences
    And all these incessant insults
    I will raise my fist
    Even higher, even further
    
    Aiming for the Moon doesn't scare me
    Even with wear and tear, I still believe in it and in my heart
    Sacrifices, if necessary I will make them
    I've done it before, but always with my fist raised
    `
    },
    {
    id: 7,
    title: "Stayin Alive",
    artist: "Bee Gees",
    audioUrl: "/songs/day7.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Eh bien, ça se voit à la façon dont j'utilise ma marche
    Je suis un homme à femmes, pas le temps de parler
    La musique forte et les femmes chaleureuses
    J'ai été frappé depuis ma naissance
    
    Et maintenant tout va bien, tout va bien
    Et tu peux regarder de l'autre côté
    Mais nous pouvons essayer de comprendre
    L'effet du New York Times sur l'homme
    
    Que tu sois frère ou que tu sois mère
    Tu restes en vie, tu restes en vie
    Sentez la ville se briser et tout le monde tremble
    Et nous restons en vie, restons en vie
    Ah, ah, ah, ah
    Rester en vie, rester en vie
    Ah, ah, ah, ah
    Rester en vie
    Oh, quand tu marches
    `
    },
    {
    id: 8,
    title: "Dernière danse",
    artist: "Kyo",
    audioUrl: "/songs/day8.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    I traveled her body for a long time
    Touched his face a hundred times
    I found gold and even
    A few stars while wiping her tears
    I learned by heart
    The purity of its forms
    Sometimes I still draw them, she does
    Part of me
    
    I just want one last dance
    Before the shadows and indifference
    A dizziness then silence
    I just want one last dance
    `
    },
    {
    id: 9,
    title: "I will survive",
    artist: "Gloria Gaynor",
    audioUrl: "/songs/day9.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Au début j'avais peur, j'étais pétrifié
    Je pensais que je ne pourrais jamais vivre sans toi à mes côtés
    Mais ensuite j'ai passé tellement de nuits à penser à quel point tu m'avais fait du mal
    Et je suis devenu fort et j'ai appris à m'entendre
    
    Vas-y maintenant, vas-y, sors par la porte
    Retourne-toi maintenant, parce que tu n'es plus le bienvenu
    N'est-ce pas toi qui as essayé de me faire du mal en me disant au revoir ?
    Pensais-tu que je m'effondrerais ? Pensais-tu que je m'allongerais et mourrais ?
    
    Oh non, pas moi, je survivrai
    Oh, tant que je sais aimer, je sais que je resterai en vie
    J'ai toute ma vie à vivre et j'ai tout mon amour à donner
    Et je survivrai, je survivrai, hé hé
    
    `
    },
    {
    id: 10,
    title: "Sous le vent",
    artist: "Garou & Céline Dion",
    audioUrl: "/songs/day10.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    And if you think I was scared, that's not true
    I give my heart a vacation, a little rest
    And if you think I was wrong, wait
    Breathe a little of the golden breath that pushes me forward
    
    And pretend I've gone to sea
    I took out the mainsail and slipped downwind
    Act like I'm leaving the earth
    I found my star, I followed it for a moment
    Downwind
    
    And if you think it's over, never
    It's just a break, a respite from the dangers
    And if you think I'm forgetting you, listen
    Open your port to the winds of the night, close your eyes
    
    And pretend I've gone to sea
    I took out the mainsail and slipped downwind
    Act like I'm leaving the earth
    I found my star, I followed it for a moment
    Downwind
    `
    },
    {
    id: 11,
    title: "Femme like U",
    artist: "K-Maro",
    audioUrl: "/songs/day11.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    When you sing, I forget, I no longer have the slightest worry
    I have evil that flees, you give sound to my life
    And then I don't know what's going on
    You have this look in your face
    Which brings me back to square one, where I left
    Takes us back to the bar party, when we went out
    And it is this same complicity that sets in
    Oh when we're on stage and we shine under the same star
    When your voice meets mine, when I have your soul in my veins
    May my vibe flow into yours
    Woman, you are beautiful and when you sing you are sexy
    Flash on her, rock star, baby
    
    Ohhh, ohhh... Yeah
    
    Give me your heart, baby
    Your body, baby hey
    Give me your good old funk
    Your rock, baby
    Your soul, baby hey
    Sing with me, I want a woman like you
    To take me to the ends of the world
    A woman like you, hey, hey!
    `
    },
    {
    id: 12,
    title: "Let it be",
    artist: "The Beatles",
    audioUrl: "/songs/day12.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Quand je me retrouve dans des moments difficiles
    Mère Marie vient à moi
    Dire des paroles de sagesse
    Qu'il en soit ainsi
    Et à mon heure d'obscurité
    Elle est debout juste devant moi
    Dire des paroles de sagesse
    Qu'il en soit ainsi
    
    Qu'il en soit ainsi, qu'il en soit ainsi
    Qu'il en soit ainsi, qu'il en soit ainsi
    Chuchotez des paroles de sagesse
    Qu'il en soit ainsi
    
    Et quand les gens au cœur brisé
    Vivre dans le monde d'accord
    Il y aura une réponse
    Qu'il en soit ainsi
    Car même s'ils peuvent être séparés
    Il y a encore une chance qu'ils voient
    Il y aura une réponse
    Qu'il en soit ainsi
    
    Qu'il en soit ainsi, qu'il en soit ainsi
    Qu'il en soit ainsi, qu'il en soit ainsi
    Ouais, il y aura une réponse
    Qu'il en soit ainsi
    Qu'il en soit ainsi, qu'il en soit ainsi
    Qu'il en soit ainsi, qu'il en soit ainsi
    Chuchotez des paroles de sagesse
    Qu'il en soit ainsi
    `
    },
    {
    id: 13,
    title: "Libertine",
    artist: "Mylène Farmer",
    audioUrl: "/songs/day13.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Moon ash, little bubble of foam
    Driven by the wind, I burn and I catch a cold
    Between my dunes lie my misfortunes
    It is naked that I learn virtue
    
    I, I am libertine
    I'm a whore
    I, I'm so fragile
    Hold my hand
    
    Splitting the moon, kisses of thorn and feather
    Lulled by a little wind, I wander
    Life is sad like a glass of grenadine
    To love is to cry when you bow
    
    I, I am libertine
    I'm a whore
    I, I'm so fragile
    Hold my hand
    `
    },
    {
    id: 14,
    title: "Lemon Tree",
    artist: "Fools Garden",
    audioUrl: "/songs/day14.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Je suis assis ici dans la pièce ennuyeuse
    C'est juste un autre dimanche après-midi pluvieux
    Je perds mon temps, je n'ai rien à faire
    Je traîne, je t'attends
    Mais rien ne se passe jamais
    Et je me demande
    
    Je conduis dans ma voiture
    Je conduis trop vite, je conduis trop loin
    j'aimerais changer de point de vue
    Je me sens si seul, je t'attends
    Mais rien ne se passe jamais
    Et je me demande
    
    Je me demande comment, je me demande pourquoi
    Hier, tu m'as parlé du ciel bleu, bleu
    Et tout ce que je peux voir c'est juste un citronnier jaune
    Je tourne la tête de haut en bas
    Je me retourne, je me retourne, je me retourne, je me retourne, je me retourne
    Et tout ce que je peux voir c'est juste un autre citronnier
    `
    },
    {
    id: 15,
    title: "Billie Jean",
    artist: "Michael Jackson",
    audioUrl: "/songs/day15.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Les gens m'ont toujours dit : "Fais attention à ce que tu fais
    Ne brise pas le cœur des jeunes filles" (hee-hee)
    Et ma mère m'a toujours dit : "Fais attention à qui tu aimes
    Et fais attention à ce que tu fais (oh-oh)
    Parce que le mensonge devient la vérité" (oh-oh), hé-hé
    
    Billie Jean n'est pas mon amante
    C'est juste une fille qui prétend que c'est moi (oh, bébé)
    Mais le gamin n'est pas mon fils (hoo)
    Elle dit que c'est moi qui suis (oh, bébé)
    Mais le gamin n'est pas mon fils (hee-hee-hee, non-non, hee-hee-hee, hoo)
    `
    },
    {
    id: 16,
    title: "Démons de minuit",
    artist: "Images",
    audioUrl: "/songs/day16.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Deserted street
    Last cigarette
    Nothing moves anymore
    Just a bar that lights up the sidewalk
    With a red neon
    I need
    To find someone
    I don't want to sleep
    I'm looking for a little warmth
    To put in my heart
    
    They drag me away at the end of the night
    The midnight demons
    They lead me to insomnia
    The ghosts of boredom
    
    In my glass
    I look at the sea
    Who swings
    I want a record
    By Funky Music
    It has to dance
    I love this girl
    On stiletto heels
    Who sways
    It adds a little warmth
    Deep in my heart
    You might also like
    meet the grahams
    Kendrick Lamar
    Si Antes Te Hubiera Conocido
    KAROL G
    Normal Thing
    Gracie Abrams
    They drag me away at the end of the night
    The midnight demons
    They lead me to insomnia
    The ghosts of boredom
    `
    },
    {
    id: 17,
    title: "Pour que tu m'aimes encore",
    artist: "Céline Dion",
    audioUrl: "/songs/day17.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    I understood all the words
    I understood correctly, thank you
    Reasonable and new
    This is how it is around here
    That things have changed
    That the flowers have faded
    Than the time before
    It was the time before
    What if everything fades and gets tired
    Loves also pass
    
    You need to know
    
    I will seek your heart
    If you take it elsewhere
    Even if in your dances
    Others dance your hours
    I will seek your soul
    In the cold, in the flames
    I will cast spells on you
    So that you love me again
    So that you love me again
    `
    },
    {
    id: 18,
    title: "Like a prayer",
    artist: "Madonna",
    audioUrl: "/songs/day18.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    La vie est un mystère
    Tout le monde doit rester seul
    Je t'entends appeler mon nom
    Et on se sent comme à la maison
    
    Quand tu appelles mon nom
    C'est comme une petite prière
    Je suis à genoux
    Je veux t'y emmener
    À minuit
    Je peux sentir ton pouvoir
    Tout comme une prière
    Tu sais que je t'y emmènerai
    
    `
    },
    {
    id: 19,
    title: "Wannabe",
    artist: "Spice Girls",
    audioUrl: "/songs/day19.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Si tu veux mon avenir
    Oublie mon passé
    Si tu veux venir avec moi
    Mieux vaut faire vite
    Maintenant, ne gaspille pas
    Mon temps précieux
    Ressaisissez-vous, tout ira bien pour nous
    
    Je te dirai ce que je veux
    Ce que je veux vraiment, vraiment
    Alors dis-moi ce que tu veux
    Ce que tu veux vraiment, vraiment
    Je veux (Hé !), je veux (Hé !)
    Je veux (Hé !), je veux (Hé !)
    Je veux vraiment, vraiment, vraiment "zig-a-zig", ah
    `
    },
    {
    id: 20,
    title: "Freed from desire",
    artist: "Gala",
    audioUrl: "/songs/day20.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Mon amour n'a pas d'argent, il a de fortes convictions
    Mon amour n'a aucun pouvoir, il a ses fortes convictions
    Mon amour n'a pas de renommée, il a de fortes convictions
    Mon amour n'a pas d'argent, il a de fortes convictions
    
    Je veux de plus en plus, les gens veulent juste de plus en plus
    Liberté et amour, ce qu'il recherche
    Je veux de plus en plus, les gens veulent juste de plus en plus
    Liberté et amour, ce qu'il recherche
    
    Libéré du désir, esprit et sens purifiés
    Libéré du désir, esprit et sens purifiés
    Libéré du désir, esprit et sens purifiés
    Libéré du désir
    `
    },
    {
    id: 21,
    title: "Toxic",
    artist: "Britney Spears",
    audioUrl: "/songs/day21.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Bébé, tu ne vois pas que j'appelle ?
    Un gars comme toi devrait porter un avertissement
    C'est dangereux, je tombe
    Il n'y a pas d'échappatoire, je ne peux pas attendre
    J'ai besoin d'un coup, bébé, donne-le-moi
    Tu es dangereux, j'adore ça
    
    Trop haut, je ne peux pas descendre
    Je perds la tête, je tourne en rond
    Est-ce que tu me sens maintenant ?
    
    Avec un avant-goût de tes lèvres, je pars en balade
    Tu es toxique, je me glisse dessous
    Avec un goût de paradis empoisonné
    je suis accro à toi
    Ne sais-tu pas que tu es toxique ?
    Et j'aime ce que tu fais
    Ne sais-tu pas que tu es toxique ?
    `
    },
    {
    id: 22,
    title: "Nuit de folie",
    artist: "Nuit de folie",
    audioUrl: "/songs/day22.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    There is no season for music to live
    Basically there are no seasons for the sound to live
    When you walk you give a cadence to your steps
    You feel the music at your fingertips
    You say that the life you were given was made for this
    So many things thanks to sound you will know
    Your heart is a pickup sapphire
    We found decibels in your check-up
    
    
    And you sing sing sing that refrain that pleases you
    And you tap, tap, tap, it's your way of loving
    This rhythm that carries you through the night
    Awaken in you the whirlwind of a wind of madness
    And you sing sing sing that refrain that pleases you
    And you tap, tap, tap, it's your way of loving
    This rhythm that carries you through the night
    Awaken in you the whirlwind of a wind of madness
    `
    },
    {
    id: 23,
    title: "Je marche seul",
    artist: "Jean-Jacques Goldman",
    audioUrl: "/songs/day23.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Like a boat drifts
    Without purpose and without motive
    I walk in the city
    All alone and anonymous
    Oh the city and its traps
    These are my privileges
    I am rich in this
    But you can't buy it
    
    I don't care, I don't care
    Of everything
    Of these chains that hang around my neck
    I run away, I forget
    I offer myself a parenthesis, a reprieve
    
    I walk alone
    In the streets that give
    And the night forgives me
    I walk alone
    Forgetting the hours
    I walk alone
    Without witnesses, without anyone
    Let my resounding footsteps
    I walk alone
    Actor and voyeur
    `
    },
    {
    id: 24,
    title: "La vie en rose",
    artist: "Edith Piaf",
    audioUrl: "/songs/day24.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Eyes that make mine look down
    A laugh that is lost on his mouth
    Here is the portrait without retouching
    Of the man I belong to
    
    When he takes me in his arms
    Let him speak to me quietly
    I see life in pink
    He says words of love to me
    Everyday words
    And that does something to me
    
    He entered my heart
    A part of happiness
    Of which I know the cause
    It's him for me, me for him in life
    He told me, swore it, for life
    And as soon as I see it
    So I feel inside me
    My beating heart
    `
    },
    {
    id: 25,
    title: "Alors on danse",
    artist: "Stromae",
    audioUrl: "/songs/day25.mp3",
    imageHint: "https://images.unsplash.com/photo-1512909006721-3d6018887383?w=800",
    lyricsHint: `
    Who says studies says work
    Who says work tells you money
    Who says money says expenses
    Who says credit says debt
    Who says debt, says bailiff to you
    Said to him sitting in the shit
    Who says love, says kids
    Says always and says divorce
    Who says loved ones, tells you mourning
    Because problems don't come alone
    Who says crisis, says world
    Says famine and says third world
    Who says fatigue says awakening
    Still deaf from yesterday
    So we go out to forget all the problems
    
    So we dance
    So we dance
    So we dance
    So we dance
    So we dance
    So we dance
    So we dance
    So we dance
    `
    }
];